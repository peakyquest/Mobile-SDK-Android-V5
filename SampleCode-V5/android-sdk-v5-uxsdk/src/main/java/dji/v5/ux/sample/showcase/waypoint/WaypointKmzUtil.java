...
/**
 * KMZ / template helpers for waypoint missions (adapted from sample {@code KMZTestUtil}).
 */
public final class WaypointKmzUtil {
    ...
    private WaypointKmzUtil() {
    }

    public static WaylineMission createWaylineMission() {
        WaylineMission waylineMission = new WaylineMission();
        waylineMission.setCreateTime((double) System.currentTimeMillis());
        waylineMission.setUpdateTime((double) System.currentTimeMillis());
        return waylineMission;
    }

    public static WaylineMissionConfig createMissionConfig(WaypointMissionGlobals missionGlobalModel) {
        WaylineMissionConfig config = new WaylineMissionConfig();
        config.setFlyToWaylineMode(DEF_WAYLINE_MODE);
        config.setFinishAction(missionGlobalModel.finishAction);
        ...
        config.setExitOnRCLostType(missionGlobalModel.lostAction);
    ...
    public static List<WaylineActionGroup> transformActionsFrom(List<WaypointPlanItem> waypointInfoModels) {
    ...
    public static WaylineActionInfo transTakePhoto() {
        ...
        ActionTakePhotoParam param = new ActionTakePhotoParam(
                0, true, photoTypes, "uxsdk_waypoint");
        param.setPayloadPositionIndex(0);
    ...
    private static WaylineActionInfo transStartRecord() {
        ...
        ActionStartRecordParam param = new ActionStartRecordParam(0,
                true, photoTypes, "uxsdk_waypoint");
